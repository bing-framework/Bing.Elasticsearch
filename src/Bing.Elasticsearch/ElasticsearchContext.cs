...
public class ElasticsearchContext : IElasticsearchContext
{
    ...
    public IObservable<ScrollAllResponse<TDocument>> ScrollAll<TDocument>(Time timeout, int numberOfReplicas, Func<ScrollAllDescriptor<TDocument>, IScrollAllRequest> selector = null, CancellationToken cancellationToken = default)
        where TDocument : class
    {
        if (numberOfReplicas < 2)
            throw new ArgumentOutOfRangeException(nameof(numberOfReplicas), "numberOfReplicas 不能小于2");
        var observable = _client.ScrollAll<TDocument>(timeout, numberOfReplicas, selector, cancellationToken);
        return observable;
    }
    ...
    /// <param name="selector">查询表达式</param>
    /// <param name="cancellationToken">取消令牌</param>
    public IEnumerable<TDocument> ScrollAll<TDocument>(int timeout, int numberOfReplicas, TimeSpan maximumRunTime, Func<ScrollAllDescriptor<TDocument>, IScrollAllRequest> selector = null, CancellationToken cancellationToken = default)
        where TDocument : class
    {
    ...
    /// <param name="cancellationToken">取消令牌</param>
    public IEnumerable<TDocument> ScrollAll<TDocument>(Func<SearchDescriptor<TDocument>, ISearchRequest> selector, int timeout = 10, double maximumRunTime = 20, int maxDegreeOfParallelism = 3, CancellationToken cancellationToken = default)
        where TDocument : class
    {
        if (maxDegreeOfParallelism < 2)
            throw new ArgumentOutOfRangeException(nameof(maxDegreeOfParallelism), "maxDegreeOfParallelism 不能小于2");
        var result = ScrollAll<TDocument>(timeout, maxDegreeOfParallelism, TimeSpan.FromMinutes(maximumRunTime), sc => sc.MaxDegreeOfParallelism(maxDegreeOfParallelism).Search(selector), cancellationToken);
        return result;
    }
    ...
    /// <returns></returns>
    public async Task<CountResponse> CountAsync<TDocument>(Func<CountDescriptor<TDocument>, ICountRequest> selector = null, CancellationToken cancellationToken = default)
        where TDocument : class
    {
