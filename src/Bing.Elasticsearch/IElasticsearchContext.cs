...
public interface IElasticsearchContext
{
    ...
    /// <param name="selector">查询表达式</param>
    /// <param name="cancellationToken">取消令牌</param>
    IObservable<ScrollAllResponse<TDocument>> ScrollAll<TDocument>(Time timeout, int numberOfReplicas,
        Func<ScrollAllDescriptor<TDocument>, IScrollAllRequest> selector = null,
    ...
    /// <param name="selector">查询表达式</param>
    /// <param name="cancellationToken">取消令牌</param>
    IEnumerable<TDocument> ScrollAll<TDocument>(int timeout, int numberOfReplicas, TimeSpan maximumRunTime,
        Func<ScrollAllDescriptor<TDocument>, IScrollAllRequest> selector = null,
        CancellationToken cancellationToken = default)
        where TDocument : class;
    ...
    /// <param name="cancellationToken">取消令牌</param>
    IEnumerable<TDocument> ScrollAll<TDocument>(Func<SearchDescriptor<TDocument>, ISearchRequest> selector,
        int timeout = 10, double maximumRunTime = 20, int maxDegreeOfParallelism = 3,
        CancellationToken cancellationToken = default) where TDocument : class;
