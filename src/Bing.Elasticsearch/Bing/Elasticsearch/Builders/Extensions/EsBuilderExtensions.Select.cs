...
public static partial class EsBuilderExtensions
{
    ...
    /// <param name="source">源</param>
    /// <param name="column">列名，范例：x=>appId</param>
    public static IEsBuilder<TEntity> Select<TEntity>(this IEsBuilder<TEntity> source, Expression<Func<TEntity, object>> column)
        where TEntity : class
    {
        source.CheckNull(nameof(source));
        if (source is IEsPartAccessor accessor)
    ...
    /// <param name="source">源</param>
    /// <param name="column">列名，范例：x=>appId</param>
    public static IEsBuilder<TEntity> RemoveSelect<TEntity>(this IEsBuilder<TEntity> source, Expression<Func<TEntity, object>> column)
        where TEntity : class
    {
        source.CheckNull(nameof(source));
        if (source is IEsPartAccessor accessor)
