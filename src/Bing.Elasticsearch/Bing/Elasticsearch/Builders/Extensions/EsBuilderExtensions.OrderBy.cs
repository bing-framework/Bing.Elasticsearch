...
public static partial class EsBuilderExtensions
{
    ...
    /// <param name="desc">是否倒排</param>
    /// <param name="appendKeyword">是否追加keyword关键词。text 类型字段不能直接使用，否则将会产生异常</param>
    public static IEsBuilder<TEntity> OrderBy<TEntity>(this IEsBuilder<TEntity> source, Expression<Func<TEntity, object>> column, bool desc = false, bool appendKeyword = false)
        where TEntity : class
    {
        if (source == null)
            throw new ArgumentNullException(nameof(source));
    ...
    /// <param name="column">排序列。范例：t => t.Name</param>
    /// <param name="appendKeyword">是否追加keyword关键词。text 类型字段不能直接使用，否则将会产生异常</param>
    public static IEsBuilder<TEntity> OrderByDescending<TEntity>(this IEsBuilder<TEntity> source, Expression<Func<TEntity, object>> column, bool appendKeyword = false)
        where TEntity : class
    {
        return OrderBy(source, column, true, appendKeyword);
    }
    ...
    /// <param name="column">排序列。范例：t => t.Name</param>
    /// <param name="appendKeyword">是否追加keyword关键词。text 类型字段不能直接使用，否则将会产生异常</param>
    public static IEsBuilder<TEntity> OrderByAscending<TEntity>(this IEsBuilder<TEntity> source, Expression<Func<TEntity, object>> column, bool appendKeyword = false)
        where TEntity : class
    {
        return OrderBy(source, column, false, appendKeyword);
    }
